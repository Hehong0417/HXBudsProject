...
// kLMChineseT         @"zh-Hant"
    ...
    _WPEditorConfiguration.enableImageSelect =(ZSSRichTextEditorImageSelect )(ZSSRichTextEditorImageSelectPhotoLibrary |ZSSRichTextEditorImageSelectTakePhoto |
                                               ZSSRichTextEditorImageSelectInsertNetwork);

   
    self.delegate = self;
//    self.itemTintColor = [UIColor colorWithRed:72 green:210 blue:213 alpha:1];
    self.itemTintColor = APP_COMMON_COLOR;
...
-(void)initView{
    ...
    [button setFrame:CGRectMake(20, 2.0, 44.0, 25.0)];
//    [button setImageEdgeInsets:UIEdgeInsetsMake(13,0, 13, 34)];
    [button addTarget:self action:@selector(backAction:) forControlEvents:UIControlEventTouchUpInside];
    [button setTitle:@"取消" forState:UIControlStateNormal];
    [button setTitleColor:LineDeepColor forState:UIControlStateNormal];
    button.titleLabel.font = FONT(13);
//    [button lh_setCornerRadius:3 borderWidth:1 borderColor:LineLightColor];
//    [button setImage:[UIImage imageNamed:@"back.png"] forState:UIControlStateNormal];
    ...
    [pulishBtn setFrame:CGRectMake(SCREEN_WIDTH - 64, 2.0, 44.0, 25.0)];
//  [pulishBtn setImageEdgeInsets:UIEdgeInsetsMake(13,0, 13, 34)];
    [pulishBtn addTarget:self action:@selector(publishedAction:) forControlEvents:UIControlEventTouchUpInside];
    [pulishBtn setTitle:@"发布" forState:UIControlStateNormal];
    [pulishBtn setTitleColor:LineLightColor forState:UIControlStateNormal];
    [pulishBtn lh_setCornerRadius:3 borderWidth:1 borderColor:LineLightColor];
    pulishBtn.titleLabel.font = FONT(13);
    UIBarButtonItem *rightBarButtonItem = [[UIBarButtonItem alloc]initWithCustomView:pulishBtn];
...
- (void)keyboardWillShow:(NSNotification *)notification {
    ...
    keyboardValue = [[userInfo objectForKey:UIKeyboardAnimationDurationUserInfoKey]doubleValue]; // 返回系统键盘弹出的动画速度
    
}
- (void)cancelAction:(UIButton *)btn {
    
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(keyboardValue * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
