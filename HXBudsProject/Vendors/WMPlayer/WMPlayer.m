...
//    self.bottomView.backgroundColor = [UIColor colorWithWhite:0.4 alpha:0.4];
    ...
    //拖拽手势
    UIPanGestureRecognizer *dragTap = [[UIPanGestureRecognizer alloc] initWithTarget:self action:@selector(actiondragTapGesture:)];
     dragTap.delegate = self;
    [self.progressSlider addGestureRecognizer:dragTap];
...
//
- (void)actiondragTapGesture:(UIPanGestureRecognizer *)sender {
...
- (void)actionPanTapGesture:(UIPanGestureRecognizer *)sender{
    ...
    CGPoint locationPoint = [sender locationInView:self.contentView];
    
    CGPoint translatedPoint = [sender translationInView:self.contentView];
    

    NSLog(@"locationPoint:%@,translatedPoint:%@",NSStringFromCGPoint(locationPoint),NSStringFromCGPoint(translatedPoint));
    
    if (sender.state == UIGestureRecognizerStateBegan) {
        
        CGPoint startPoint = [sender locationInView:self.contentView];
        //触摸开始, 初始化一些值
        _hasMoved = NO;
