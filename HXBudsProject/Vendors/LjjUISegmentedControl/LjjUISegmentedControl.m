...
//        NSLog(@"这里defont%@",[button.titleLabel.font familyName]);
        ...
        WEAK_SELF();
        button = [XYQButton ButtonWithFrame:CGRectMake(i*witdFloat, 0, witdFloat, self.bounds.size.height-2) imgaeName:@"ic_sanjiao" titleName:SegumentArray[i] contentType:LeftTitleRightImage buttonFontAttributes:[FontAttributes fontAttributesWithFontColor:self.titleColor fontsize:self.titleFont.pointSize] tapAction:^(XYQButton *button) {
...
//        [button setBackgroundColor:[UIColor redColor]];
        button.center_X = witdFloat*i + 0.5*witdFloat;
        [button setTag:i];
        //
        buttonDownWidth = button.bounds.size.width+30;
        if (i==0) {
